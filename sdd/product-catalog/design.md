...
- **Scope**: Two new static routes — `/productos` (catalog with filters/sort/pagination) and `/producto/[slug]` (detail page with gallery, specs, power calculator, related products, WhatsApp CTA, SEO)
- **Business model alignment**: WhatsApp is the primary CTA on every product; no cart, no checkout, no auth
...
- Static presets: `[{load: "Router (5W)", watts: 5}, {load: "Laptop (65W)", watts: 65}, {load: "Fan (25W)", watts: 25}, {load: "Lights (10W)", watts: 10}]`
...
- Uses `pagination.page`, `pagination.totalPages`, `pagination.next`, `pagination.prev`
...
## Threat Matrix
N/A — no routing, shell, subprocess, VCS/PR automation, or executable-file classification boundary in this change.

---

## Migration / Rollout
No migration required. New routes added alongside existing. No breaking changes to existing pages or components.

---

## Open Questions

1. **Product image paths in data**: Currently `/images/placeholder-*.webp` (404→onerror). Switch to `src/assets/` imports for real WebP/AVIF via `astro:assets`?
2. **Category filter pills vs checkboxes UX**: Desktop — pills, Mobile — accordion? (Tailwind responsive variant)

---

## Engram Persistence
Saved as `sdd/product-catalog/design` with `topic_key: sdd/product-catalog/design`, type: `architecture`, `capture_prompt: false`.