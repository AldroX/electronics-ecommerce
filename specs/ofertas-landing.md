...
> **Note on OfferCard**: The existing `OfferCard` links to `/producto/{productSlug}`. For the landing page, we need cards that link to `/ofertas/{slug}`. Options: (a) add a `linkTarget` prop to `OfferCard`, (b) create a new `OfferCardLanding` variant, (c) use a wrapper component. Decision deferred to design phase.