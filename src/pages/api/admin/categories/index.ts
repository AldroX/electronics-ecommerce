...
import { z } from 'zod';
import { CategoryCreateSchema } from '@/lib/api/validation';
...
export const POST: APIRoute = async (context) => {
  try {
    ...
    const bodyResult = CategoryCreateSchema.safeParse(await context.request.json());
    ...
    const insertResult = await supabase.from('categories').insert(insertData).select().single();

    if (insertResult.error) {
      console.error('Supabase error creating category:', insertResult.error);
      return new Response(
        JSON.stringify({ ok: false, error: Errors.internalError('Error al crear categoría') }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }

    const data = insertResult.data as CategoryRow;
