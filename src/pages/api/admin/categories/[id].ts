...
import { z } from 'zod';
import { CategoryUpdateSchema } from '@/lib/api/validation';
...
export const PATCH: APIRoute = async (context) => {
  try {
    ...
    const bodyResult = CategoryUpdateSchema.safeParse(await context.request.json());
    ...
    if (fetchError || !existing) {
      return new Response(
        JSON.stringify({ ok: false, error: Errors.notFound('Categoría no encontrada') }),
    ...
    if (updateResult.error) {
      console.error('Supabase error updating category:', updateResult.error);
      return new Response(
        JSON.stringify({ ok: false, error: Errors.internalError('Error al actualizar categoría') }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }

    const data = updateResult.data as CategoryRow;
...
export const DELETE: APIRoute = async (context) => {
  try {
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ ok: false, error: Errors.internalError('Error al eliminar categoría') }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
