/// ============================================================
/// POST /api/admin/kits - Create kit (admin only)
/// Task 3.6
/// ============================================================
...
import { z } from 'zod';
import { KitCreateSchema } from '@/lib/api/validation';
...
export const POST: APIRoute = async (context) => {
  try {
    ...
    const bodyResult = KitCreateSchema.safeParse(await context.request.json());
    ...
    const insertResult = await (supabase.from('kits').insert(insertData) as any).select().single();

    if (insertResult.error) {
      console.error('Supabase error creating kit:', insertResult.error);
      return new Response(
        JSON.stringify({ ok: false, error: Errors.internalError('Error al crear kit') }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }

    const data = insertResult.data as KitRow;
    ...
    console.error('Admin create kit error:', err);
    return new Response(
